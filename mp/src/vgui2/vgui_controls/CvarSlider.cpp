...
{ 
    return new CvarSlider(nullptr, nullptr, nullptr, 2);
}
...
void CvarSlider::SetPrecision(int precision)
{
    ...
    {
        Q_snprintf(m_szPrecisionFormat, sizeof(m_szPrecisionFormat), "%s.%i%s", "%", precision, "f");
    }
    m_fScaleFactor = powf(10, precision);
}
